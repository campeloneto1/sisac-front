"use client";

import Link from "next/link";
import { useMemo, useState } from "react";
import { Plus } from "lucide-react";

import { useAuth } from "@/contexts/auth-context";
import { usePermissions } from "@/hooks/use-permissions";
import { useRanks } from "@/hooks/use-ranks";
import { hasPermission } from "@/lib/permissions";
import { Button } from "@/components/ui/button";
import { Card, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Pagination } from "@/components/ui/pagination";
import { Skeleton } from "@/components/ui/skeleton";
import { RanksFilters } from "@/components/ranks/filters";
import { RanksTable } from "@/components/ranks/table";

export function RanksListPage() {
  const { user } = useAuth();
  const permissions = usePermissions("ranks");
  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);
  const filters = useMemo(
    () => ({
      page,
      per_page: 15,
      search: search || undefined,
    }),
    [page, search],
  );
  const ranksQuery = useRanks(filters);

  if (!hasPermission(user, "administrator") || !permissions.canViewAny) {
    return (
      <Card className="border-slate-200/70 bg-white/80">
        <CardHeader>
          <CardTitle>Acesso negado</CardTitle>
          <CardDescription>Você precisa de `administrator` e `ranks.viewAny` para visualizar postos/graduações.</CardDescription>
        </CardHeader>
      </Card>
    );
  }

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="font-display text-3xl text-slate-900">Postos/graduações</h1>
          <p className="text-sm text-slate-500">
            Gerencie a hierarquia administrativa usada em promoções e históricos funcionais.
          </p>
        </div>

        {permissions.canCreate ? (
          <Button asChild>
            <Link href="/ranks/create">
              <Plus className="mr-2 h-4 w-4" />
              Novo posto/graduação
            </Link>
          </Button>
        ) : null}
      </div>

      <RanksFilters
        search={search}
        onSearchChange={(value) => {
          setSearch(value);
          setPage(1);
        }}
        onClear={() => {
          setSearch("");
          setPage(1);
        }}
      />

      {ranksQuery.isLoading ? (
        <div className="space-y-3">
          <Skeleton className="h-24 w-full" />
          <Skeleton className="h-24 w-full" />
        </div>
      ) : ranksQuery.isError ? (
        <Card className="border-slate-200/70 bg-white/80">
          <CardHeader>
            <CardTitle>Erro ao carregar postos/graduações</CardTitle>
            <CardDescription>Verifique a API, o resource e as permissões do usuário autenticado.</CardDescription>
          </CardHeader>
        </Card>
      ) : !ranksQuery.data?.data.length ? (
        <Card className="border-slate-200/70 bg-white/80">
          <CardHeader>
            <CardTitle>Nenhum posto/graduação encontrado</CardTitle>
            <CardDescription>Crie um novo registro ou refine a busca por nome e sigla.</CardDescription>
          </CardHeader>
        </Card>
      ) : (
        <div className="space-y-4">
          <RanksTable ranks={ranksQuery.data.data} />
          <Pagination
            currentPage={ranksQuery.data.meta.current_page}
            lastPage={ranksQuery.data.meta.last_page}
            total={ranksQuery.data.meta.total}
            from={ranksQuery.data.meta.from}
            to={ranksQuery.data.meta.to}
            onPageChange={setPage}
            isDisabled={ranksQuery.isFetching}
          />
        </div>
      )}
    </div>
  );
}
