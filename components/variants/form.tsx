  ...
  async function onSubmit(values: VariantFormValues) {
    ...
    if (mode === "create") {
      const response = await createMutation.mutateAsync(
        payloadBase satisfies CreateVariantDTO,
      );
  ...
  return (
    <Card className="border-slate-200/70 bg-white/80">
      <CardHeader>
        ...
        <CardDescription>
          Modelos ficam dentro de Administrador e se vinculam a uma marca para
          compor os catalogos do sistema.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form
        ...
        >
          ...
          <div className="flex justify-end gap-3 md:col-span-2">
            <Button asChild variant="ghost">
              <Link
                href={
                  mode === "create" ? "/variants" : `/variants/${variant?.id}`
                }
              >
                Cancelar
              </Link>
