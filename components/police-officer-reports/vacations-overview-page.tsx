...
import { usePoliceOfficerVacationsOverviewReport } from "@/hooks/use-police-officer-reports";
...
export function PoliceOfficerVacationsOverviewReportPage() {
  const { user } = useAuth();
  ...
  const reportQuery = usePoliceOfficerVacationsOverviewReport(filters, Boolean(activeSubunit) && hasPermission(user, "reports") && permissions.canViewAny);
  ...
  if (!activeSubunit) {
    return <Card className="border-slate-200/70 bg-white/80"><CardHeader><CardTitle>Selecione uma subunidade</CardTitle><CardDescription>Escolha uma subunidade ativa antes de gerar o relatório de férias.</CardDescription></CardHeader></Card>;
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-[24px] border border-slate-200/70 bg-white/80 p-6">
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          <div><h1 className="font-display text-3xl text-slate-900">Férias</h1><p className="mt-2 text-sm text-slate-600">Acompanhamento paginado das férias por ano de referência, situação e validade.</p></div>
        </div>
      </div>
      ...
      {reportQuery.isLoading ? (
        ...
        <Card className="border-slate-200/70 bg-white/80"><CardHeader><CardTitle>Erro ao gerar relatório</CardTitle><CardDescription>Não foi possível carregar as férias no momento.</CardDescription></CardHeader></Card>
      ) : !reportQuery.data?.data.length ? (
        <Card className="border-slate-200/70 bg-white/80"><CardHeader><CardTitle>Nenhuma férias encontrada</CardTitle><CardDescription>Ajuste os filtros para ampliar o recorte do relatório.</CardDescription></CardHeader></Card>
      ) : (
                  ...
                  {reportQuery.data.data.map((item) => (
                    <tr key={item.id} className="border-t border-slate-200/70">
                      ...
                      <td className="px-4 py-4 text-slate-700">
                        <p>Disponível: {item.available_days ?? 0}</p>
                        <p className="text-xs text-slate-500">Restante: {item.remaining_days ?? 0}</p>
                      </td>
                      <td className="px-4 py-4 text-slate-700">
