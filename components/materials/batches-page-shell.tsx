...
import { Archive, ArrowLeft, Boxes, ShieldAlert } from "lucide-react";
...
interface MaterialBatchesPageShellProps {
  title: string;
  description: string;
...
export function MaterialBatchesPageShell({
  title,
  description,
  requiredPermission,
  showIntegrationNotice = false,
  children,
}: MaterialBatchesPageShellProps) {
    ...
    return (
      <Card className="border-slate-200/70 bg-white/80">
        <CardHeader>
          ...
          <CardDescription>Você não possui permissão suficiente para acessar a gestão de lotes deste material.</CardDescription>
        </CardHeader>
      </Card>
    );
  }
        ...
        <Card className="border-amber-200/80 bg-amber-50/80">
          <CardHeader>
            ...
            <CardDescription className="text-amber-800">
              O backend já devolve lotes dentro do `Material.show`, mas ainda não expoe endpoints dedicados para CRUD de `MaterialBatch`.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-2 text-sm text-amber-900">
            <p>Esta area já consegue listar e detalhar lotes usando o payload do material.</p>
